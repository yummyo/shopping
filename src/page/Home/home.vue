      ...
      <section class="w mt30 clearfix" v-for="(item,i) in floors" :key="i">
        <y-shelf :title="item.title">
          <div slot="content" class="floors">
            <div class="imgbanner">
              <a v-bind:href="floors[i].image.link">
...
export default {
  ...
  methods: {
    ...
    bgMove(dom, eve) {
      ...
      dom.style['-webkit-transform'] = `rotateY(${X / 50}deg) rotateX(${Y /
        50}deg)`
...
.bg {
  ...
  height: 500px;
  margin: 20px auto;
  background: url('http://47.92.240.201/media/banner/banner_4zTRRnw.jpg') center
    no-repeat;
